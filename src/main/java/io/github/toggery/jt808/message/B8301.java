 ...
 * @author togger
 */
public class B8301 extends AbstractToStringJoiner implements Codec {
    ...
    public void encode(int version, ByteBuf buf) {
        ...
        Codec.writeCountHeadedContent(buf, IntUnit.BYTE, events, (b, that) -> {
            int count = 0;
            for (final Event event : that) {
                if (event == null) continue;
                event.encode(version, b);
                count++;
            }
            return count;
        });
     ...
     * @author togger
     */
    public static class Event extends AbstractToStringJoiner implements Codec {
        ...
        public void encode(int version, ByteBuf buf) {
            ...
            Codec.writeString(buf, IntUnit.BYTE, content);
        }

        @Override
        public void decode(int version, ByteBuf buf) {
            id = Codec.readByte(buf);
            content = Codec.readString(buf, IntUnit.BYTE);
        }
