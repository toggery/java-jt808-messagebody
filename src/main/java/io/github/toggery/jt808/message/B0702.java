 ...
 * @author togger
 */
public class B0702 extends AbstractToStringJoiner implements Codec {
    ...
    public void encode(int version, ByteBuf buf) {
        ...
        Codec.writeString(buf, IntUnit.BYTE, name);
        // 后补 0x00（左对齐）
        Codec.writeChars(buf, licenseNo, -20, PadChar.NUL);
        Codec.writeString(buf, IntUnit.BYTE, authority);
        Codec.writeBcd(buf, validity, 4);

        if (version > 0) {
            // 后补 0x00（左对齐）
            Codec.writeChars(buf, idCardNo, -20, PadChar.NUL);
        }
    }
    ...
    public void decode(int version, ByteBuf buf) {
        ...
        name = Codec.readString(buf, IntUnit.BYTE);
        licenseNo = Codec.readChars(buf, 20);
        authority = Codec.readString(buf, IntUnit.BYTE);
        validity = Codec.readBcd(buf, 4, false);

        if (version > 0) {
            idCardNo = Codec.readChars(buf, 20);
        }
    }
